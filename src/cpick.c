...
static void *cpick_code[] = {
	...
	TABLE(2, 8),
	TLLABELx(_("Red"), 0, 0, 0, 0, 10),
	REF(inputs[CPICK_INPUT_RED]),
	TLXSPIN(c.input_vals[CPICK_INPUT_RED], 0, 255, 1, 0),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Green"), 0, 1, 0, 0, 10),
	REF(inputs[CPICK_INPUT_GREEN]),
	TLXSPIN(c.input_vals[CPICK_INPUT_GREEN], 0, 255, 1, 1),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Blue"), 0, 2, 0, 0, 10),
	REF(inputs[CPICK_INPUT_BLUE]),
	TLXSPIN(c.input_vals[CPICK_INPUT_BLUE], 0, 255, 1, 2),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Hue"), 0, 3, 0, 0, 10),
	REF(inputs[CPICK_INPUT_HUE]),
	TLXSPIN(c.input_vals[CPICK_INPUT_HUE], 0, 1529, 1, 3),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Saturation"), 0, 4, 0, 0, 10),
	REF(inputs[CPICK_INPUT_SATURATION]),
	TLXSPIN(c.input_vals[CPICK_INPUT_SATURATION], 0, 255, 1, 4),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Value"), 0, 5, 0, 0, 10),
	REF(inputs[CPICK_INPUT_VALUE]),
	TLXSPIN(c.input_vals[CPICK_INPUT_VALUE], 0, 255, 1, 5),
		EVENT(CHANGE, cpick_spin_change),
	TLLABELx(_("Hex"), 0, 6, 0, 0, 10),
	EXEC(add_hex),
// !!! Or better just to hide it?
	IFx(opacity_f, 1),
		TLLABELx(_("Opacity"), 0, 7, 0, 0, 10),
		REF(inputs[CPICK_INPUT_OPACITY]),
		TLXSPIN(c.input_vals[CPICK_INPUT_OPACITY], 0, 255, 1, 7),
			EVENT(CHANGE, cpick_spin_change),
	ENDIF(1),
	WDONE, // table
	WDONE, // xhbox
